...
std::vector<double> KNNClassifier::predict(const std::vector<std::vector<double>>& X_test) const {
    ...
    for (const auto& testPoint : X_test) {
        ...
        for (size_t i = 0; i < X_train_.size(); i++) {
            ...
            distances.push_back(std::make_pair(dist, y_train_[i]));
        }
        ...
        for (int i = 0; i < k_; i++) {
            double vecino = distances[i].second; 
            labelCounts[vecino]++;                 
        }

        double bestLabel = -1.0;
        int bestCount = -1;
        for (const auto& par : labelCounts) {
            if (par.second > bestCount) {
                bestCount = par.second;   
                bestLabel = par.first;   
            }
        }

        y_pred.push_back(bestLabel);
    }
